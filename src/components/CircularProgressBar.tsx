...
function CircularProgressBar({ size, episode, isPickleLive = false }: CircularProgressBarProps) {
  ...
  // 라이브는 보라색 그라데이션
  const LIVE_COLOR = 'url(#liveGradient)';

  // Pickle Live면 기본 원만 그림
  if (isPickleLive) {
    return (
      <svg width={size} height={size} className="rotate-[11deg]">
        <defs>
          <linearGradient id="liveGradient">
            <stop offset="0%" stopColor="#FF696C" /> {/* 0% */}
            <stop offset="50%" stopColor="#F635F9" /> {/* 50% */}
            <stop offset="100%" stopColor="#F4004D" /> {/* 100% */}
          </linearGradient>
        </defs>
